...
1. **Why we need science** - the limits of intuition and everyday reasoning
2. **Cognitive biases and logical fallacies** - confirmation bias, pattern-seeing, cherry-picking
3. **Famous cautionary tales** - Clever Hans and why controlled observation matters
4. **What science is** - deduction, induction, falsification, and the scientific method
:::
...
#| echo: false
#| out-width: "40%"
#| out-extra: "style=\"float:right; padding:15px\""
...
#| echo: false
#| out-width: "50%"
#| out-extra: "style=\"float:centre; padding:10px\""
...
#| echo: false
#| label: "W1_TB1"
checkdown::check_question("About 50%",
               options = c("About 50%",
                 "About 6% (23/365)",
                 "Practically 0%",
                 "About 23%"),
               ...
               right = "Correct! Counter-intuitively, a class of just 23 students already gives a roughly 50-50 chance of a shared birthday. Our numeric intuition handles addition well but fails badly at compound probabilities - see the full calculation below.",
               wrong = "Most people guess far too low a probability - our intuition is poor at compound probabilities. Try the calculation in the box below.")
```
...
#| echo: false
#| label: "W1_TB2"
checkdown::check_question("Nearly 100%",
               options = c("Nearly 100%",
                 "About 73%",
                 "About 50%",
                 "About 20% (73/365)"),
               type = "radio",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! With 73 students there are 2,628 possible pairs, and the probability of at least one shared birthday rises to 99.96% - effectively certain. The R calculation below shows how to compute this.",
               wrong = "Think about how many *pairs* 73 students form - the probability compounds across all of them. See the calculation below.")
```
...
The probability of 2 students having the same birthday in a class of 73 students is almost 1 (or 100 percent).
:::
...
#| fig-align: "center"
#| out-width: "65%"
#| fig-cap: "[Part 1 of a video about Skinner's experiments with Pigeons](https://www.youtube.com/watch?v=QKSvu3mj-14)."
...
#| fig-align: "center"
#| out-width: "65%"
#| fig-cap: "[Part 2 of a video about Skinner's experiments with Pigeons](https://www.youtube.com/watch?v=erhmslcHvaw)."
...
#| echo: false
#| label: "W1_TB3"
checkdown::check_question("Pareidolia - our tendency to perceive meaningful patterns (e.g. faces) in random stimuli",
               options = c("Pareidolia - our tendency to perceive meaningful patterns (e.g. faces) in random stimuli",
                 "Slow, deliberate thinking as described by Kahneman",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Pareidolia leads people to interpret random images or patterns of light and shadow as faces or figures - one of several pattern-seeing biases that can underlie supernatural experiences.",
               wrong = "Think of the face on Mars: which bias makes us see meaningful patterns where there are none?")
```
...
#| echo: false
#| label: "W1_TB4"
checkdown::check_question("A single anecdote tells us nothing about the general relationship between the factors - we do not know how representative the case is",
               options = c("A single anecdote tells us nothing about the general relationship between the factors - we do not know how representative the case is",
                 "The grandfather's age was not verified by a doctor",
                 "The claim involves two habits, but anecdotes can only cover one",
                 "Nothing - one clear counter-example falsifies the claim that smoking is harmful"),
               type = "radio",
               q_id = "W1_TB4",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Questions about how one factor affects another concern general patterns, not individual cases. Anecdotes are unrepresentative and biased samples; answering such questions requires systematic, unbiased observation. (And note: 'smoking harms health' is a probabilistic claim, so one healthy smoker does not falsify it.)",
               wrong = "Is one unusual case informative about a general pattern? What would you need instead of an anecdote?")
```
...
#| fig-align: "center"
#| out-width: "65%"
#| fig-cap: "[Clever Hans](https://www.youtube.com/watch?v=rVCNzjhBZyQ)."
...
#| echo: false
#| label: "W1_Q1"
checkdown::check_question("She only looks for evidence that confirms her belief and ignores potential counter-evidence",
               options = c("She only looks for evidence that confirms her belief and ignores potential counter-evidence",
                 "She should have studied monolingual children only",
                 "Creativity cannot be studied scientifically",
                 "Her sample of children is too old"),
               type = "radio",
               q_id = "W1_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! This is confirmation bias combined with cherry-picking: collecting only confirming cases tells us nothing about whether the claim holds in general. A proper test would systematically compare bilingual and monolingual children and actively look for disconfirming evidence.",
               wrong = "Think about what evidence she collects - and, more importantly, what evidence she never looks for.")
```
...
#| echo: false
#| label: "W1_Q2"
checkdown::check_question("Apparent abilities can result from unintended cues, so claims must be tested under controlled conditions",
               options = c("Apparent abilities can result from unintended cues, so claims must be tested under controlled conditions",
                 "Animals can learn arithmetic if trained long enough",
                 "The audience deliberately deceived the researchers",
                 "Observations by multiple witnesses are always reliable"),
               type = "radio",
               q_id = "W1_Q2",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Hans was responding to subtle, unintentional cues from his questioners (posture, tension), not doing arithmetic. Only when the questioner did not know the answer did Hans fail - showing why controlled conditions are essential for testing claims.",
               wrong = "Hans succeeded when the questioner knew the answer and failed when the questioner did not. What does that tell you?")
```
...
#| echo: false
#| label: "W1_Q3"
checkdown::check_question("All vowels are voiced; /a/ is a vowel; therefore /a/ is voiced",
               options = c("All vowels are voiced; /a/ is a vowel; therefore /a/ is voiced",
                 "I have observed 50 voiced vowels, so all vowels are probably voiced",
                 "/a/ is voiced, so vowels in other languages are likely voiced too",
                 "Most vowels I know are voiced, so voicing is common"),
               type = "radio",
               q_id = "W1_Q3",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Deduction moves from general premises to a specific, logically necessary conclusion: if the premises are true, the conclusion must be true. The other options are inductive - they generalize from specific observations and their conclusions are only probable.",
               wrong = "Deduction moves from the general to the specific with logical necessity; induction generalizes from observations.")
```
