# Significance, Probability and the Normal Distribution {#week3}
...
1. **Significance and probability** - what *p*-values are (and are not)
2. **Probability calculations** - binomial probabilities with coin-toss examples in R
3. **Alpha and beta errors** - false positives, false negatives, and statistical power
4. **Independence** of observations and why it matters
5. **Corrections for multiple testing** (e.g. Bonferroni)
6. **The normal distribution** - properties, skewness, kurtosis, and z-scores
:::
...
ggplot(df, aes(x = w100, y = p100, fill = col)) +
  ...
  geom_text(
    data = label_df,
    aes(x = w100, y = p100, label = label),
    parse = TRUE,          # interprets as math notation
    inherit.aes = FALSE
  )
```
...
df <- df |>
  mutate(
    cump = cumsum(p100),          # cumulative probability
    highlight = cump >= 0.95      # flag values beyond 95%
  )

# Store the label as a string, not an expression
label_df <- data.frame(
  w100 = 70,
  p100 = 0.05,
  label = "mu[1] >= mu[2]"
...
ggplot(df, aes(x = w100, y = p100, fill = highlight)) +
  geom_col() +
  scale_fill_manual(values = c("gray", "red")) +
  labs(
    x = "Frequency Count (Head)", 
...
checkdown::check_question("16",
               ...
               right = "Correct! Each toss has 2 outcomes and the tosses are independent, so there are 2 x 2 x 2 x 2 = 2^4 = 16 possible sequences (HHHH, HHHT, HHTH, ...). Try listing them in a table as we did for three tosses above.",
               wrong = "Each toss doubles the number of possible sequences: 2 tosses give 4, 3 tosses give 8, and 4 tosses give ...?")
```
...
#| echo: false
#| label: "W3_TB2"
checkdown::check_question("About 27%",
               options = c("About 27%",
                 "About 43% (3 out of 7)",
                 "About 50%",
                 "About 3%"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! In R: sum(dbinom(3, 7, 0.5)) = 0.273. There are 35 sequences with exactly 3 heads out of 128 possible sequences: 35/128 = .273.",
               wrong = "In R: sum(dbinom(3, 7, 0.5)). Count the sequences with exactly 3 heads and divide by all 2^7 = 128 sequences.")
```
...
#| echo: false
#| label: "W3_TB3"
checkdown::check_question("About 33%",
               options = c("About 33%",
                 "About 27%",
                 "About 7%",
                 "About 66%"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! In R: sum(dbinom(c(2, 5), 7, 0.5)) = 0.328. Both outcomes have 21 sequences each: (21 + 21)/128 = .328.",
               wrong = "Add the probabilities of the two outcomes. In R: sum(dbinom(c(2, 5), 7, 0.5)).")
```
...
#| echo: false
#| label: "W3_TB4"
checkdown::check_question("About 23%",
               options = c("About 23%",
                 "About 50%",
                 "About 71% (5 out of 7)",
                 "About 5%"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! In R: sum(dbinom(5:7, 7, 0.5)) = 0.227. Sequences with 5, 6, or 7 heads: (21 + 7 + 1)/128 = .227.",
               wrong = "Sum the probabilities for 5, 6, and 7 heads. In R: sum(dbinom(5:7, 7, 0.5)).")
```
...
#| echo: false
#| label: "W3_TB5"
checkdown::check_question("About 77%",
               options = c("About 77%",
                 "About 50%",
                 "About 57% (4 out of 7)",
                 "About 23%"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! In R: sum(dbinom(3:6, 7, 0.5)) = 0.766. Sequences with 3, 4, 5, or 6 heads: (35 + 35 + 21 + 7)/128 = .766.",
               wrong = "Sum the probabilities for 3, 4, 5, and 6 heads. In R: sum(dbinom(3:6, 7, 0.5)).")
```
...
In R, we use the `skewness()` function from the **e1071** package. This function can calculate skewness in three slightly different ways, depending on which formula is used.  
...
1. **Type 1 (older textbooks)**  
\[
g_1 = \frac{m_3}{m_2^{3/2}}
\]  
Here \(m_2\) and \(m_3\) are the 2nd and 3rd *central moments* (ways of measuring spread and asymmetry).  

2. **Type 2 (SPSS and SAS)**  
\[
G_1 = g_1 \cdot \sqrt{\frac{n(n-1)}{(n-2)}}
\]  
This adjusts the Type 1 formula to correct for bias when the sample size \(n\) is small.  

3. **Type 3 (MINITAB and BMDP)**  
\[
...
zdf |>
  ...
  flextable::fontsize(size = 12, part = "header") |>
  flextable::align_text_col(align = "center") |>
  flextable::set_caption(caption = "Results of two tests at university (class 1 had a maximum score of 80, class 2 had a maximum score of 100).")  |>
  flextable::border_outer()
```
...
The z-score of student B in class 2 (who scored 70 out of 100) would be calculated as 70 minus `r mean(zdf$Score_c2)` (the mean score of class 2) divided by `r round(sd(zdf$Score_c2), 3)` (the standard deviation of class 2). This would result in a z-score of `r round((70 - mean(zdf$Score_c2)) / sd(zdf$Score_c2), 3)` which means that student B's score is `r round((70 - mean(zdf$Score_c2)) / sd(zdf$Score_c2), 3)` standard deviations *above* the mean of class 2.

\begin{equation}
  z_{studentB} =\frac{70 - `r mean(zdf$Score_c2)`} {`r round(sd(zdf$Score_c2), 3)`} 
  = `r round((70 - mean(zdf$Score_c2)) / sd(zdf$Score_c2), 3)`
\end{equation}

The z-scores show that the two students differ substantially relative to their classes: student A performed below the average of class 1 (indicated by the negative z-score), while student B performed above the average of class 2 (indicated by the positive z-score). Because student A's z-score is clearly lower than student B's, student A has performed worse compared to student B - and the z-scores allow us to state this precisely, even though the two tests used different scales.
...
zdf |>
  ...
  flextable::fontsize(size = 12, part = "header") |>
  flextable::align_text_col(align = "center") |>
  flextable::set_caption(caption = "Scores, means, standard deviations, and z-scores of two tests at university.")  |>
  flextable::border_outer()
```
...
#| echo: false
#| label: "W3_Q1"
checkdown::check_question("If the null hypothesis were true, data this extreme (or more extreme) would occur in about 3% of studies",
               options = c("If the null hypothesis were true, data this extreme (or more extreme) would occur in about 3% of studies",
                 "There is a 3% probability that the null hypothesis is true",
                 "The effect is present in 3% of the population",
                 "There is a 97% probability that the alternative hypothesis is true"),
               type = "radio",
               q_id = "W3_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! The p-value is a conditional probability: it describes how likely the data are *given* the null hypothesis - not how likely any hypothesis is given the data.",
               wrong = "The p-value is a statement about the probability of the *data* under the null hypothesis, not about the probability of any hypothesis.")
```
...
#| echo: false
#| label: "W3_Q2"
checkdown::check_question("About 1",
               options = c("About 1",
                 "None - if there are no real effects, no test will be significant",
                 "About 5",
                 "Exactly 20"),
               type = "radio",
               q_id = "W3_Q2",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! At α = .05, each test has a 5% false-positive rate, so 20 tests yield about 20 × .05 = 1 spurious significant result on average. This is the multiple-testing problem, and why corrections such as the Bonferroni correction exist.",
               wrong = "Each test has a 5% chance of a false positive. Multiply that by the number of tests.")
```
...
#| echo: false
#| label: "W3_Q3"
checkdown::check_question("About 95%",
               options = c("About 95%",
                 "About 50%",
                 "About 68%",
                 "Exactly 99%"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! In a normal distribution, roughly 68% of values fall within one standard deviation of the mean, about 95% within two, and about 99.7% within three - a pattern worth memorizing, as it underlies confidence intervals and significance testing.",
               wrong = "Remember the 68-95-99.7 rule for one, two, and three standard deviations.")
```
