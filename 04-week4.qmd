# Getting Started with R and RStudio {#week4}
...
1. **Why R?** - advantages of code-based, reproducible analysis
2. **Installing and setting up** R, RStudio, and R projects with a sensible folder structure
3. **The RStudio interface** - console, script editor, environment, and help panes
4. **First steps in R** - objects, functions, operators, and running commands from scripts
5. **Packages** - installing them once, loading them every session
6. **Working with data** - loading a first data set and inspecting it
:::
...
The session preparation section of your notebook or script will thus also state which packages a script relies on.
...
checkdown::check_question("<-",
               ...
               right = "Correct! `x <- 5` stores the value 5 in the object `x`. (The equals sign `=` also works for assignment, but `<-` is the widely used convention in R.)",
               wrong = "It looks like an arrow pointing from the value to the object name.")
```
...
#| echo: false
#| label: "W4_Q2"
checkdown::check_question("install.packages() downloads a package once; library() loads an installed package in each new session",
               options = c("install.packages() downloads a package once; library() loads an installed package in each new session",
                 "They do exactly the same thing",
                 "library() downloads packages; install.packages() loads them",
                 "install.packages() must be run every time you start R"),
               type = "radio",
               q_id = "W4_Q2",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Installation is a one-time download to your computer; loading with library() makes the package's functions available and must be done in every new R session that uses the package.",
               wrong = "One of the two is a one-time download; the other must be repeated in every session.")
```
...
#| echo: false
#| label: "W4_Q3"
checkdown::check_question("The dplyr package has not been loaded with library(dplyr) in this session",
               options = c("The dplyr package has not been loaded with library(dplyr) in this session",
                 "mutate() does not exist in R",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! An installed package is not automatically available - you must load it with library() in each session before using its functions (or call the function with its package prefix, e.g. dplyr::mutate()).",
               wrong = "Installed and loaded are not the same thing - what must you do at the start of each session?")
```
