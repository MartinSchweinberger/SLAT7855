# Working with Tables in R and RStudio {#week5}

::: {.callout-tip}
## What This Session Covers

This session teaches you to work with tabular data in R - the bread and butter of quantitative analysis.

1. **Functions, objects, and data inspection** - `head()`, `str()`, `summary()`
2. **Accessing data** - rows, columns, and individual cells; tabulating with `table()`
3. **Saving and loading data** - text files and Excel spreadsheets
4. **The tidyverse workflow** - renaming, piping (`|>`), filtering, and ordering data
5. **Creating and changing variables** - `mutate()` and if-statements
6. **Summarizing** - `group_by()` and `summarise()`
7. **Reshaping data** - long vs. wide formats (gathering and spreading)
:::
...
The pipe `|>` passes the result of one function into the next and can be read as *and then*: `data |> filter(Age == \"old\") |> summarise(mean(Score))` means *take the data, and then keep only old speakers, and then compute the mean score*. Pipes let you write analyses as readable left-to-right sequences instead of nesting functions inside one another - compare the unpiped equivalent `summarise(filter(data, Age == \"old\"), mean(Score))`, which must be read inside-out.
:::
...
**Q1. What does the pipe symbol |> do?**

```{r}
#| echo: false
#| label: "W5_Q1"
checkdown::check_question("It passes the result of the expression on its left as input to the function on its right",
               options = c("It passes the result of the expression on its left as input to the function on its right",
                 "It compares two values for equality",
                 "It creates a new column in a table",
                 "It deletes rows from a table"),
               type = "radio",
               q_id = "W5_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! The pipe can be read as 'and then': it hands the output of one step to the next step, letting you chain operations into readable sequences.",
               wrong = "Read the pipe as 'and then' - what does it hand over from one step to the next?")
```

---

**Q2. Which function keeps only the rows of a table that meet a condition?**

```{r}
#| echo: false
#| label: "W5_Q2"
checkdown::check_question("filter()",
               options = c("filter()",
                 "select()",
                 "mutate()",
                 "arrange()"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! filter() subsets rows by condition (e.g. filter(Age == \"old\")). select() chooses columns, mutate() creates or changes columns, and arrange() orders rows.",
               wrong = "One function works on rows by condition; select() works on columns; mutate() creates columns; arrange() sorts.")
```

---

**Q3. You want a first quick look at a freshly loaded data set: its column names, data types, and a few values. Which function is most informative?**

```{r}
#| echo: false
#| label: "W5_Q3"
checkdown::check_question("str()",
               options = c("str()",
                 "print()",
                 "c()",
                 "library()"),
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! str() displays the structure of an object: dimensions, column names, data types, and example values - the fastest way to check whether data loaded as expected. head() and summary() are useful complements.",
               wrong = "You need a function that shows the *structure* of the object.")
```
