...
This week, we focus on how to describe and summarize data [see e.g. @bickel2012descriptive; @thompson2009descriptive]. This tutorial is aimed at beginners and intermediate users of R with the aim of showcasing how to summarize data using R. The aim is not to provide a fully-fledged analysis but rather to show and exemplify selected useful methods for summarizing data. 
...
Confidence intervals (CIs) give a range thatâ€™s likely to include a population value with a certain degree of confidence. As such, CIs tell us how likely it is to get a value within a certain range if we drew another sample from the same population. 
...
The Â´CIÂ´ function provides the mean reaction time (`r tt[2]`) and the 95 percent confidence band. With 95 percent confidence, the mean reaction time will have a mean between `r round(tt[3], 2)`  and `r round(tt[1], 2)` milliseconds (ms).
