# Introduction to Data Visualization {#week7}

::: {.callout-tip}
## What This Session Covers

This session introduces data visualization with ggplot2 - how to build informative, publication-ready graphs layer by layer.

1. **The ggplot framework** - the grammar of graphics: data, aesthetics, geoms, layers
2. **Basic principles** - what makes a visualization honest and informative
3. **Building graphs** - scatterplots, line graphs, histograms, bar plots, and boxplots
4. **Customizing** - axes, titles, symbols, text annotations, themes, and legends
:::
...
To be able to generate `ggplot` plots, you need to install and load the `ggplot2` package. We also install and load the `gridExtra` package (for arranging several plots in one panel) and the `dplyr` package.
...
ggplot2 builds every plot from the same three ingredients: **data** (a table), **aesthetic mappings** (`aes()` - which variables map onto which visual properties: x, y, color, shape, size), and **geoms** (the geometric objects that display the data: `geom_point()`, `geom_line()`, `geom_boxplot()`, ...). Layers are added with `+`. Once you understand this grammar, every new plot type is just a new combination of the same building blocks - not a new function to memorize.
:::
...
**Q1. In ggplot2, what is the role of aes()?**

```{r}
#| echo: false
#| label: "W7_Q1"
checkdown::check_question("It maps variables in the data onto visual properties such as x, y, and color",
               options = c("It maps variables in the data onto visual properties such as x, y, and color",
                 "It chooses the color palette of the plot",
                 "It saves the plot to a file",
                 "It filters the data before plotting"),
               type = "radio",
               q_id = "W7_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! aes() defines the aesthetic mappings - which variable determines position on the x-axis, y-axis, the coloring, and so on. The geoms then draw those mappings.",
               wrong = "aes() connects *variables* to *visual properties* - it does not itself draw or style anything.")
```

---

**Q2. You want to display the distribution of a single numeric variable (e.g. essay lengths). Which geom is most appropriate?**

```{r}
#| echo: false
#| label: "W7_Q2"
checkdown::check_question("geom_histogram()",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Histograms bin a continuous variable and show how many observations fall into each bin, revealing the shape of the distribution. geom_bar() is for counting categories, not binning continuous values.",
               wrong = "You need a geom that bins a *continuous* variable and displays frequencies of the bins.")
```

---

**Q3. What does the box in a boxplot represent?**

```{r}
#| echo: false
#| label: "W7_Q3"
checkdown::check_question("The interquartile range - the middle 50% of the data, with the median as a line inside the box",
               options = c("The interquartile range - the middle 50% of the data, with the median as a line inside the box",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! The box spans from the 1st quartile (25th percentile) to the 3rd quartile (75th percentile), so it contains the middle half of the data. The line inside is the median; the whiskers extend to the most extreme values within 1.5 IQR, and points beyond are plotted as potential outliers.",
               wrong = "The box shows quartiles, not means or standard deviations.")
```
