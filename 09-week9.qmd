# Simple Linear Regression {#week9}
...
1. **The basic principle** - fitting a line that minimizes squared residuals
2. **Assumptions** - linearity, normality of residuals, homoscedasticity, independence
3. **Example 1** - preposition use across real time (numeric predictor)
4. **Model diagnostics** - `performance::check_model()` and formal checks
5. **Example 2** - teaching styles (categorical predictor)
6. **Interpreting and reporting** - coefficients, R², model comparison, and summary tables
:::
...
We will now check the model assumptions using diagnostic plots. To do this, we use the `check_model` function from the `performance` package [@performance] which generates a panel of diagnostic plots and - very conveniently - tells us in the subtitle of each panel what we should be looking for.
...
performance::check_outliers(m1.lm)
```
...
performance::check_normality(m1.lm)
# formal test of heteroscedasticity
performance::check_heteroscedasticity(m1.lm)
```

In our case, the diagnostics do not indicate any severe violations of the model assumptions and we can proceed with the interpretation of the model.
...
#| echo: false
#| label: "W9_Q1"
checkdown::check_question("For each one-unit increase in word frequency, predicted reaction time decreases by 25 ms",
               options = c("For each one-unit increase in word frequency, predicted reaction time decreases by 25 ms",
                 "Reaction times are 25% faster for frequent words",
                 "The model explains 25% of the variance",
                 "25 observations were excluded from the model"),
               type = "radio",
               q_id = "W9_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! The slope is the predicted change in the outcome per one-unit change in the predictor. The negative sign means higher frequency predicts faster (lower) reaction times.",
               wrong = "The slope tells you how much the *outcome* changes when the *predictor* increases by one unit.")
```
...
#| echo: false
#| label: "W9_Q2"
checkdown::check_question("The difference between an observed value and the value predicted by the model",
               options = c("The difference between an observed value and the value predicted by the model",
               ...
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Residuals are the model's errors for the individual observations - and both model fitting (least squares) and assumption checking operate on them.",
               wrong = "Think of the vertical distance between a data point and the regression line.")
```
...
#| echo: false
#| label: "W9_Q3"
checkdown::check_question("Homoscedasticity (constant variance of residuals)",
               options = c("Homoscedasticity (constant variance of residuals)",
                 "Normality of the predictor",
                 "Independence of the predictor and the intercept",
                 "The assumption that R² is above .5"),
               type = "radio",
               q_id = "W9_Q3",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! A funnel shape means the residual variance grows with the fitted values - heteroscedasticity. Standard errors and p-values become unreliable; transformations or robust standard errors can help.",
               wrong = "A funnel-shaped residual plot indicates that the residual *variance* is not constant.")
```
