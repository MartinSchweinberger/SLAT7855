...
1. **Decision trees and conditional inference trees** - recursive partitioning for classification
2. **Random forests** - ensemble methods, training/test splits, and variable importance
3. **Machine learning as an outlook** - the two cultures: prediction vs. explanation
4. **Large language models and generative AI** - how LLMs work, from n-gram models to transformers, and how they build on the statistical concepts of this course
5. **Implications for the field** - opportunities, risks, and why statistical literacy matters more than ever
:::
...
#| echo: false
#| label: "W13_Q1"
checkdown::check_question("The tree has overfitted: it has learned the noise in the training data rather than generalizable patterns",
               options = c("The tree has overfitted: it has learned the noise in the training data rather than generalizable patterns",
                 "The new data must contain errors",
                 "99% accuracy proves the model is excellent",
                 "The tree needs more branches to improve"),
               type = "radio",
               q_id = "W13_Q1",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! A large gap between training and test performance is the signature of overfitting. The remedy is not a more complex model but constraints on complexity, ensembles (e.g. random forests), and evaluation on held-out data.",
               wrong = "Compare performance on seen vs. unseen data - what does a large gap between them signal?")
```
...
#| echo: false
#| label: "W13_Q2"
checkdown::check_question("To evaluate the model on data it has never seen, obtaining an honest estimate of how well it generalizes",
               options = c("To evaluate the model on data it has never seen, obtaining an honest estimate of how well it generalizes",
                 "To make the analysis run faster",
                 "Because R cannot process complete data sets",
                 "To remove outliers from the data"),
               type = "radio",
               q_id = "W13_Q2",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! Evaluating a model on its own training data is like grading students on questions they memorized the answers to - the held-out test set provides an unbiased assessment of predictive performance on new cases.",
               wrong = "What would be wrong with testing the model on the very data it was fitted to?")
```
...
#| echo: false
#| label: "W13_Q3"
checkdown::check_question("It repeatedly predicts a probability distribution over the next token and samples from it",
               options = c("It repeatedly predicts a probability distribution over the next token and samples from it",
                 "It looks up answers in a database of facts",
                 "It retrieves and recombines stored sentences verbatim",
                 "It translates all input into formal logic and derives conclusions"),
               type = "radio",
               q_id = "W13_Q3",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct! LLMs are (very large) statistical models of token sequences: they estimate conditional probability distributions for the next token and generate text by sampling from these distributions - which also explains both their fluency and their ability to produce confident falsehoods.",
               wrong = "Think back to the bigram model we built - what did generation consist of there?")
```
